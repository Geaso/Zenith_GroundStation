...
rem
rem  产出：
rem    dist\ZenithGroundStation\              自包含目录
rem    dist\ZenithGroundStation-<日期>.exe    单文件自解压包（发这个给客户）
rem    dist\ZenithGroundStation-<日期>.zip    zip 备选
rem
rem  目标机器无需安装 Qt、无需 VC++ 运行库。
rem
rem  刻意采用【白名单】策略：先建空目录，再逐项放入明确需要的东西。
rem  绝不从工程目录整体拷贝 —— 那样迟早会把 .git / build / src 一起发出去。
rem
rem  本文件必须以 GBK 编码保存。UTF-8 批处理会让 cmd 解析器错乱。
rem ===========================================================================
...
rem mingw 运行库：windeployqt 不一定带全，显式补上
...
if errorlevel 1 goto :fail
copy /y "%PROJ_DIR%\config\radio_pairings.json" "%DIST_DIR%\config\" >nul
if errorlevel 1 goto :fail
...
rem 单文件自解压包。注意 7-Zip 官方的 7z.sfx 只做解压，不支持解压后自动运行
rem （那需要第三方的 7zSD.sfx 模块），所以客户是"双击 -> 选目录 -> 解压 -> 运行"。
if exist "%SEVENZIP%\7z.exe" (
    echo       生成单文件自解压包 ...
    ...
    echo       [跳过] 未找到 7-Zip: %SEVENZIP%
    echo              只产出 zip；装好 7-Zip 后重跑即可得到单文件 exe。
)
...
echo ==========================================================
echo  交付目录: %DIST_DIR%
if exist "%SFX_PATH%" echo  单文件:   %SFX_PATH%
echo  zip:      %ZIP_PATH%
echo.
echo  交付前自检:
...
echo.
echo  客户使用方式:
echo    双击单文件 exe  -^>  选择解压目录  -^>  进入 ZenithGroundStation\
echo    双击 ZenithGroundStationQt.exe 运行
echo.
popd
endlocal
exit /b 0
